...
public class SecurityConfig {
    ...
    @Bean
    @Order(2)
    public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http, OrderedFormContentFilter formContentFilter)
            throws Exception {
        http
    ...
    // Register Frontend as a Client here (Other Microservices are Resources)
    @Bean
            ...
            if(context.getTokenType().equals(OAuth2TokenType.ACCESS_TOKEN)){
                context.getClaims().claims(claims -> {
                    if(context.getAuthorizationGrantType().equals(AuthorizationGrantType.AUTHORIZATION_CODE)){
                        ...
                        if (roles.isEmpty()){
                           roles = context.getClaims().build().getClaim("scope");
                        }
